...
export class CannonWrapperGenericProvider extends ethers.providers.Provider {
  ...
  private recordedTransactions: ethers.Transaction[];
  ...
  async sendTransaction(signedTransaction: string | Promise<string>): Promise<ethers.providers.TransactionResponse> {
    ...
    const rawTxn = await signedTransaction;
    
    // record transaction data
    this.recordedTransactions.push(ethers.utils.parseTransaction(rawTxn));

    try {
      return await this.passThroughProvider.sendTransaction(rawTxn);
    } catch (err) {
  ...
  resetRecordedTransactions(): ethers.Transaction[] {
    const recordedTransactions = this.recordedTransactions;
    this.recordedTransactions = [];
